...
PRINT_ELEMENT() {
  ...
  # Check if input is a number or character
  if [[ ! $INPUT =~ ^[0-9]+$ ]]
  then
    ATOMIC_NUMBER=$(echo $($PSQL "SELECT atomic_number FROM elements WHERE symbol=INITCAP('$INPUT') OR name=INITCAP('$INPUT');") | sed 's/ //g')
  else
    ATOMIC_NUMBER=$(echo $($PSQL "SELECT atomic_number FROM elements WHERE atomic_number=$INPUT;") | sed 's/ //g')
  fi

  # Check if the given argument is in the database
  if [[ -z $ATOMIC_NUMBER ]]
  then
